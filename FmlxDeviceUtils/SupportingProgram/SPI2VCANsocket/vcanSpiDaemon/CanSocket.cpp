...
CCanSocket::CCanSocket(ChannelType channel, int channelNum)
{
    ...
    m_pHandle->ifName = (char*)malloc (IFNAMSIZ * sizeof (char));
    char buffer [5];
    ...
    std::strncpy(m_pHandle->ifName, ifname, IFNAMSIZ);
...
int CCanSocket::createSocket()
{
    ...
    std::strncpy(ifr.ifr_name, m_pHandle->ifName, IFNAMSIZ);
